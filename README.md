...
- Ngôn ngữ: Kotlin
- UI Framework: Jetpack Compose (Giao diện người dùng khai báo Declarative UI) & Material 3
- Bản đồ và định vị: Google Maps Compose SDK & Google Play Services Location
...
- Xuất dữ liệu: Android Graphics PDF Document (PdfExporter)
...
- Android SDK: Min SDK 24 (Android 7.0), Target SDK 35 (Android 15), Compile SDK 37.
...
### Bước 3: Cấu hình Google Maps API (Tùy chọn)
Để hiển thị bản đồ vị trí phòng trọ, bạn có thể thêm khóa Google Maps API Key vào file `local.properties` tại thư mục gốc của dự án Android:
```properties
MAPS_API_KEY=your_google_maps_api_key_here
```
Nếu không cấu hình, ứng dụng vẫn hoạt động bình thường với các tính năng danh sách và bản đồ cơ bản.
...
- Tìm kiếm và lọc phòng trọ thông minh theo tỉnh thành, phường xã, khoảng giá và các tiện ích đi kèm (dữ liệu tỉnh thành tích hợp offline sẵn sàng).
...
- Đặt lịch hẹn xem phòng trực tiếp với chủ trọ.
- Đặt cọc giữ chỗ trực tuyến an toàn thông qua cổng thanh toán PayOS với cơ chế bảo vệ tiền cọc Escrow.
- Xem nội dung, ký kết hợp đồng điện tử và xuất file PDF hợp đồng trực tiếp trên ứng dụng.
- Quản lý danh sách hóa đơn hàng tháng, xem chi tiết chỉ số điện nước và thanh toán nhanh qua mã QR VietQR / PayOS.
- Đánh giá phòng, chấm điểm dịch vụ, báo cáo vi phạm và xem lịch sử uy tín của bản thân.
...
- Đăng tin cho thuê phòng, cập nhật hình ảnh, giá thuê, danh mục tiện ích và mức bồi thường tài sản.
- Xác thực danh tính chủ nhà (eKYC) bằng ảnh Căn cước công dân (mặt trước, mặt sau) và ảnh chụp chân dung.
- Quản lý lịch hẹn xem phòng từ khách thuê.
...
- Đánh giá uy tín người thuê sau khi kết thúc hợp đồng.
...
- `app/src/main/java/com/example/ezroom/data/`: Triển khai các Repository, Retrofit Remote API services, DTOs và quản lý dữ liệu offline (`assets/data/`).
- `app/src/main/java/com/example/ezroom/ui/`: Chứa các màn hình Composable (Renter, Host, Auth), components tái sử dụng, theme giao diện và cấu hình Navigation.
...
- `app/src/main/java/com/example/ezroom/util/`: Chứa các lớp tiện ích cấu hình API (ApiConfig), quản lý Token đăng nhập, định dạng tiền tệ, xuất PDF và xử lý thời gian.
